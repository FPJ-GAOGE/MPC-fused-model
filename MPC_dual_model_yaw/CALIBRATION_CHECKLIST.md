...
- [x] 平移 QP 已同步 V4 Pro1 八推进器逐台正反向限额和联合可达域；纯轴范围为
  `Fx/Fright=+-16.296831 N`、`Fdown=-23.0472..29.5236 N`。
...
- [ ] 标定真实推力曲线、正反推力不对称、电压影响、流速影响和推进器互扰；当前转换
  只采用线性对称比例。
- [ ] 检查平移与 roll/pitch/yaw 同时请求时的电机饱和。当前 QP 包含平移联合约束，
  但 yaw 力矩仍在 QP 外，尚未包含平移和 yaw 共同占用推进器的六自由度约束。
