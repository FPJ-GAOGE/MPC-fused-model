...
```text
模型1: x[k+1] = A_d x[k] + B_d (tau[k] - tau_base[k])
模型2: x[k+1] = A_d x[k] + B_d tau[k]
```

模型1把在线学习到的匹配基线力视为已解释的持续作用，只把相对基线的部分作为新作用。模型2把当前综合力全部计入预测，适合基线尚未可靠或力变化很快的阶段。它们都不是鱼加速度的直接测量模型，而是两种不同的未知运动/基础力假设。
...
当两模型的候选差异 `Delta=M1+M2-2C` 小于每个方向的噪声门槛时，不用噪声级差异更新该方向，而是向模型一的稳定基线先验恢复。当前门槛为 `(1e-7,1e-7,1e-7) m2`；这不是给控制量加补偿，而是避免匀速或静止方向被测量噪声永久留在模型二。`Delta` 明显大于门槛时仍使用上式：模型一残差更小则 `a1` 接近 `0.99`，快速变速阶段模型二残差更小则 `a1` 向 `0.01` 降低。
...
`build_default_staircase_fusion()` 是默认阶梯参数的唯一构造入口，`MPCTracker` 和 `live_integration_example.py` 均调用它，避免一个入口使用阶梯而另一个入口又回到 20×8。`FusionConfig(staircase_horizon_caps=None)` 仍保留原来的全多步历史方式，仅供显式对照实验和现有调用兼容。预测时冻结当前模型权重，但每个预测步都滚动使用上一预测步的力。
...
其中 `W=diag(a1_x,a1_y,a1_z,a1_x,a1_y,a1_z)`。`a1=1` 就是纯模型1，`a1=0` 就是纯模型2。`tau_previous` 仍只用于力变化率约束，不再参与模型选择的状态方程。
...
在 UnderwaterVision 当前直驱链路中，MPC 不把力分配成 8 路命令，而是直接发送合力；但 QP 仍使用 V4 Pro1 的 8 路物理包络约束。QP 使用的 canonical 顺序为
...
融合模型的 `force_weights` 惩罚
`tau[k]-diag(a1)tau_base`：纯模型一时等于相对基线力，纯模型二时等于绝对力。
`tau_base` 不进入模型二预测；它只由相对位置和估计相对速度在线学习，并用于模型一和求解失败/目标丢失时的安全回退。
这避免模型一把非零匹配力错误地当作需要压回零的控制代价。QP 超时或失败时，
控制器先保持上一份可行力；若还没有可行解，则限速返回锁存的基础力，而不是
直接返回零力。桥接节点的诊断还记录三轴相对速度、由相邻滤波状态得到的相对加速度、候选差异 `Delta` 和水平/垂向包络。实时图默认使用 Qt 窗口并保存 PNG；无图形显示时仍会保存 PNG。
